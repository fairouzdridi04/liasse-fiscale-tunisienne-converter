...
Le fichier `.exe` est disponible dans la section **[Releases](../../releases/latest)** de ce dépôt.

1. Allez dans l'onglet **Releases** → téléchargez `LiasseFiscale.exe`
...
> ℹ️ Le fichier `.exe` n'est pas inclus dans le code source du dépôt en raison de sa taille. Il est distribué exclusivement via les Releases GitHub.
...
├── .gitignore                          ← Exclut dist/, build/, *.exe
...
> ℹ️ Les dossiers `dist/` et `build/` sont exclus du dépôt. Le fichier `.exe` est distribué via les **[Releases GitHub](../../releases)**.
